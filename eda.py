...
monthly_agg.plot.bar(y="count", figsize=(10, 5))
...
{train_static.columns[idx]: describe(train_static.columns[idx]) for idx in range(5)}
{train_static.columns[idx]: describe(train_static.columns[idx]) for idx in range(len(train_static.columns))}

train_depth0 = train_static.join(train_base[["case_id", "target"]], on="case_id", how="left", rsuffix="_base")

col = train_static.columns[2]
print({col: describe(col)})
finebinning(
    train_depth0,
    col,
    "target",
